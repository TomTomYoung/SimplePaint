...
このブランチで、基本10ツールの常時表示、設定の整理、ブラシ見本、取り消しの修正、レイヤー付き復元、保存操作の統合、縦横比固定と余白変更を実装しました。

検証は Node.js 24.19.0 で185件成功・失敗0件です。実ブラウザの目視確認は環境制限により未実施です。変更内容、既存PRとの関係、検証範囲と残る確認は [USABILITY_IMPROVEMENTS_JA.md](USABILITY_IMPROVEMENTS_JA.md) を参照してください。

以下は以前の進捗記録です。画面構成と保存・履歴については上記の新しい記録を優先してください。
