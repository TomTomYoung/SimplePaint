import { replaceDocumentLayers, bmp, layers, activeLayer } from '../core/layer.js';
...
export async function applyLayeredSnapshot({ engine, fitToScreen, snapshot }) {
  ...
  if (!Number.isInteger(width) || !Number.isInteger(height) || width < 1 || height < 1) throw new TypeError('Invalid snapshot dimensions');
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    restored.push(applyLayerMetadata(canvas, entry, index));
  }

  // Decode everything first: a damaged image must leave the current document intact.
  const requested = Number.isInteger(snapshot.activeLayer) ? snapshot.activeLayer : 0;
  ...
  replaceDocumentLayers(restored, width, height, requested, engine);
  const toolId = engine?.store?.getState?.().toolId;
  if (toolId) {
    engine.setTool?.(toolId === 'select-free' ? 'select-rect' : toolId);
    engine.eventBus?.emit('session:restored');
  }
  fitToScreen?.();
}

export function isLayeredSnapshot(snapshot) {
  return snapshot?.version === SNAPSHOT_VERSION && Array.isArray(snapshot.layers) && snapshot.layers.length > 0;
}
